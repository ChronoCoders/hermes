...
pub fn decrypt_data(encrypted: &[u8], password: &str) -> Result<Vec<u8>> {
    let package: EncryptedPackage =
        serde_json::from_slice(encrypted).map_err(|_| HermesError::DecryptionFailed)?;

    let salt = SaltString::from_b64(&package.salt).map_err(|_| HermesError::DecryptionFailed)?;
