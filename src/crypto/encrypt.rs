    ...
    if let Some(names) = recipient_names {
        ...
        for name in names {
            ...
            if !pubkey_path.exists() {
                return Err(HermesError::ConfigError(format!("Recipient public key not found: {}", name)));
            }
        ...
        return Err(HermesError::EncryptionFailed("Either password or recipients must be provided".to_string()));
    }
